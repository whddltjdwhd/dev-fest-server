...
describe('Grader Service - 기본 검증', () => {
  test('올바른 정답에 대해 성공을 반환해야 합니다', () => {
    const correctSlots =
        calculateCorrectSlots(MASTER_SCHEDULE, MASTER_CONSTRAINTS);
...
describe('Rule #1: 강의 시간 중첩 검증', () => {
  ...
  test(
      '강의 시간과 겹치지 않는 알바 시간은 통과해야 합니다 (Rule #1만)', () => {
        const nonOverlappingSlots = [
          {day: '월', start: '12:00', end: '14:00'}  // 강의 사이 공강 시간
        ];
        const result =
            grade(nonOverlappingSlots, MASTER_SCHEDULE, MASTER_CONSTRAINTS);
...
describe('Rule #2: 이동 시간 준수 검증', () => {
  test('이동 시간을 고려하지 않은 시간은 실패해야 합니다', () => {
    ...
    const result =
        grade(noTravelTimeSlots, MASTER_SCHEDULE, MASTER_CONSTRAINTS);
    expect(result.success).toBe(false);
  ...
  test('이동 시간을 올바르게 적용한 시간은 Rule #2를 통과해야 합니다', () => {
    ...
    const result =
        grade(correctTravelTimeSlots, MASTER_SCHEDULE, MASTER_CONSTRAINTS);
...
describe('Rule #3: 최소 근무 시간 검증', () => {
  test('60분 미만의 알바 시간은 실패해야 합니다', () => {
    ...
    const result =
        grade(shortDurationSlots, MASTER_SCHEDULE, MASTER_CONSTRAINTS);
    expect(result.success).toBe(false);
  ...
  test('정확히 60분인 알바 시간은 Rule #3를 통과해야 합니다', () => {
    ...
    const result =
        grade(exactMinDurationSlots, MASTER_SCHEDULE, MASTER_CONSTRAINTS);
    // Rule #3는 통과
...
describe('Rule #5: 완전성 검증', () => {
  ...
  test('불필요한 시간대가 포함된 경우 실패해야 합니다', () => {
    const correctSlots =
        calculateCorrectSlots(MASTER_SCHEDULE, MASTER_CONSTRAINTS);
    const extraSlots = [
      ...correctSlots, {day: '화', start: '15:15', end: '15:45'}
      // 30분 (불가능한 시간)
    ];
...
describe('formatGradeResult - 결과 포맷팅', () => {
  test('성공 결과를 올바르게 포맷팅해야 합니다', () => {
    const correctSlots =
        calculateCorrectSlots(MASTER_SCHEDULE, MASTER_CONSTRAINTS);
    const gradeResult =
        grade(correctSlots, MASTER_SCHEDULE, MASTER_CONSTRAINTS);
  ...
  test('실패 결과를 올바르게 포맷팅해야 합니다', () => {
    ...
    const gradeResult =
        grade(incompleteSlots, MASTER_SCHEDULE, MASTER_CONSTRAINTS);
    const formatted = formatGradeResult(gradeResult);
